...
## External clinic-site bootstrap

`POST /api/v1/voice/web/widget-session` accepts only `{ "widgetKey": "wgt_..." }` and requires one real HTTP/HTTPS `Origin` header. The key is a public routing identifier, not a secret. Before requesting an ElevenLabs signed URL or writing Redis state, the server resolves the channel and active tenant from that key and requires the canonical request origin to exactly match the channel's `allowedOrigins`. An empty list blocks all external embedding. Unknown keys, unavailable channels or tenants, and missing, malformed, or unauthorized origins all receive the same safe `403` response.

The successful response uses the existing `signedUrl`, `voiceSessionToken`, and safe `context` contract, plus `expiresIn`; it is marked `Cache-Control: no-store`. The signed URL remains a short-lived ElevenLabs startup credential generated server-side. Keep the voice session token in memory only: never put it in local/session storage, a URL, analytics, or logs. The Redis session binds the canonical embedding origin while retaining its original absolute expiration.

Browser preflight is deliberately transport-only. For this route, a syntactically valid HTTP/HTTPS origin may preflight `POST` with `Content-Type`; credentials are disabled and `Vary: Origin` is emitted. Because `OPTIONS` has no widget key, this does not authorize an installation. The subsequent POST always performs the per-widget exact-origin check before credentials or state are created. Existing authenticated/admin routes retain the configured application CORS policy.
...
The response deliberately excludes the database UUID, `tenantId`, tenant slug, timestamps, email and phone fields, escalation numbers, internal relationships, configuration metadata, audit data, and matching scores. Any future tool that consumes `key` must revalidate `tenantId`, `status = ACTIVE`, and `locationNumber`; the key is a reference, not authorization. Matching first applies Unicode normalization, case folding, whitespace and harmless-punctuation normalization, and the explicit regional spelling equivalence `centre` → `center`. A unique full-name canonical match always wins. Only when no canonical match exists, full canonical names are compared with normalized Levenshtein similarity: automatic selection requires a score of at least `0.85` and a margin of at least `0.10` over the runner-up. A qualifying best match without that margin returns `resolved: false`, `ambiguous: true`, and at most five `{ key, name }` candidates; substring matching is not used. Clear listing questions return the same compact `{ key, name }` shape, so addresses and timezones are disclosed only after one location resolves. No match returns `resolved: false`, `ambiguous: false`, and an empty `matches` array. The resolver never falls back to the widget default after an explicit miss.
